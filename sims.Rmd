...
local <- TRUE
complexity <- FALSE
results <- rbindlist(unlist(lapply(c( 0, 0.5, 1, 2 ), function(pos){
  set.seed(123)
  ...
  lapply( c(500, 1000, 2000, 3000, 4000), function(n){
    if(!local){
    ATE <- 1
  } else {
...
for(pos_const in unique(results_summary$pos_const)) {
  ...
  p <- ggplot(tmp, aes(x= n, y = coverage, color = method, linetype = method)) + geom_line() + facet_wrap(~pos, ncol =1)+ scale_y_log10() + geom_hline(yintercept = 0.95, color = "grey") + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.5), legend.position = "none",   legend.box = "horizontal" 
