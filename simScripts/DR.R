    ...
    try({
      ...
      for(lrnr in c("gam_0", "gam_1", "gam_2", "xgboost")) {
        ...
        for(misp in c("1", "2", "3", "4")) {
          mu1 <- initial_estimators$mu1 #+ n^(-1/3)*rowMeans(abs(sin(5*W[,1:3])))
          mu0 <- initial_estimators$mu0 #- n^(-1/3)*rowMeans(abs(sin(5*W[,1:3])))
          pi1 <- initial_estimators$pi1 #+ n^(-1/3)*rowMeans(abs(sin(5*W[,1:3])))
          pi0 <- initial_estimators$pi0 #- n^(-1/3)*rowMeans(abs(sin(5*W[,1:3])))
          if(misp == "2") {
