...
fit_hal_cate <- function(W, A, Y, include_propensity_score = FALSE, weights = NULL,  max_degree = 3, num_knots = c(sqrt(length(Y)), length(Y)^(1/3), length(Y)^(1/5)), smoothness_orders = 1, family_Y = c("auto", "gaussian", "binomial", "poisson"),  family_A = c("auto", "gaussian", "binomial", "poisson"), formula_cate = NULL, max_degree_cate = max(max_degree - 1, 1), num_knots_cate = num_knots, smoothness_orders_cate = smoothness_orders,    screen_variables = TRUE, screen_control = list(), verbose = TRUE,...) {
  ...
  subset <- A==0
  fit_mu0 <- fit_hal(W_pi[subset, , drop = F], Y[subset], weights = weights[subset], max_degree = max_degree, num_knots = num_knots, smoothness_orders = smoothness_orders, family = family_Y, screen_variables = screen_variables,... )
  ...
  fit_cate <- fit_hal(W[subset, , drop = F], Y[subset] - mu0[subset], weights = weights[subset], max_degree = max_degree, num_knots = num_knots, smoothness_orders = smoothness_orders, family = "gaussian", screen_variables = screen_variables,... )
  ...
  IF_Y_map <- function(x_proj) {
    M <- solve(t(x_proj) %*% x_proj) %*% t(x_proj)

    alpha <- x_basis %*% solve(t(x_basis) %*% x_basis, t(M %*% (x_basis1 - x_basis0)  ))
    IF <- alpha * (Y - mu)
  }
