...
#'
 ...
 fit_hal_cate_partially_linear <- function(W, A, Y, weights = NULL,  pA1W = NULL, EYW = NULL, formula_cate = NULL, max_degree_cate = max(max_degree - 1, 1), num_knots_cate = num_knots, smoothness_orders_cate = smoothness_orders, screen_variables_cate = FALSE, lrnr_A= NULL, lrnr_Y = NULL,   params_hal_EYW = list(smoothness_orders =1, max_degree = 3, num_knots = c(sqrt(length(Y)), length(Y)^(1/3), length(Y)^(1/5)), screen_variables = TRUE), params_hal_pA1W =  params_hal_EYW   , verbose = TRUE,...) {
